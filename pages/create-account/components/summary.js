...
const Summary = ({currentPharmacyDetails}) => {
    ...
    const createPharmacyAccount = async () => {
        ...
        pharmacyData.append("pharmacist_license", currentPharmacyDetails?.documentation?.pharmacist_license)
        pharmacyData.append("premise_license", currentPharmacyDetails?.documentation?.premise_license)
        ...
        pharmacyData.append("pharmacyInformation", JSON.stringify(currentPharmacyDetails.pharmacyInformation))
        pharmacyData.append("bankDetails", JSON.stringify(currentPharmacyDetails.bankDetails))

        const createPharmacyAccountRequest = await fetch("http://localhost:5000/auth/signup", {
            method: "POST",
            body: pharmacyData
        })

        const createPharmacyAccountResponse = await createPharmacyAccountRequest.json()
        ...
        <div  className={caStyles.summary}>
            ...
            <table>
                <tbody>
                    <tr>
                        <td>
                            <Button type={"button"} label={"Previous"} onButtonClick={() => router.push("/create-account/bank-details")} buttonIntent={"previous"} theme={"outline"} />
                        </td>

                        <td>
                            <Button onButtonClick={() => {
                                createPharmacyAccount()
                            }} label={"Submit"} buttonIntent={"next"} theme={"solid"} />
                        </td>
                    </tr>
                </tbody>
            </table>
